...
End-to-end test that a third peer can communicate, over a rayfish **trusted**
network, with a user identity backed by **two** physical devices — an original
device and a second device paired into the same identity via a `DeviceCert`.
...
| Host  | Identity | Role |
|-------|----------|------|
| srv-a | U        | primary device, coordinator of the trusted network |
...
```bash
tests/e2e/provision.sh     # create 3 DEV1-S Ubuntu instances in fr-par-1
tests/e2e/run.sh           # deploy + pair + create/join + assert ping & ray send
tests/e2e/teardown.sh      # destroy the instances when done (manual)
```

- `provision.sh` writes `tests/e2e/.servers` (gitignored: `id ip label zone`).
  It is a no-op while that file exists; delete it to provision a fresh set.
- `run.sh` is fully re-runnable against the same `.servers`: it **resets each
  ...
  identities, no leftover `e2e` network. Just run `tests/e2e/run.sh` again.
  Pass `KEEP_STATE=1 tests/e2e/run.sh` to skip the reset (re-run against the
  existing network). It prints a `PASS`/`FAIL` line per check and exits non-zero
  if any failed.
