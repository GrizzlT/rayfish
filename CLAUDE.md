...
- `src/apply.rs` — declarative deploy spec for `ray apply` (Phase B): `DeploySpec = BTreeMap<network, NetworkSpec { trusted, firewall: SuggestedFirewall }>`, TOML load/parse/serialize (`[networks.<name>]` wrapper or flat form, `deny_unknown_fields`), `expected_hosts()` (union of subject + peer hostnames in the spec's firewall blocks), and the `EXAMPLE_SPEC` printed by `--example`. The orchestrator itself lives in `main::ipc_apply`.
